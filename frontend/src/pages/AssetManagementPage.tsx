...
const TopologicalAssetView: React.FC<{ assets: AssetMatrixRow[] }> = ({ assets }) => {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-primary/5">
        ...
        {assets.map((asset) => {
          ...
          return (
            <g key={asset.id}>
              ...
              <title>{`${asset.asset}\nStatus: ${asset.status}\nTLS: ${asset.tls_version ?? 'unknown'}\nPQC: ${asset.pqc_status ?? 'unknown'}`}</title>
            </g>
          );
        })}
      </svg>
...
const AssetManagementPage: React.FC = () => {
  const [assets, setAssets] = useState<AssetMatrixRow[]>([]);
  const [viewMode, setViewMode] = useState<'table' | 'topological'>('table');
  const [error, setError] = useState<string | null>(null);

  const loadMatrix = async () => {
    const response: any = await assetsAPI.getMatrix();
  ...
  useWebSocket<{ type: string; data?: { items?: AssetMatrixRow[] } }>({
    ...
    onMessage: (message) => {
      if (message.type === 'asset_matrix_updated' && message.data?.items) {
        setAssets(message.data.items);
        setError(null);
      }
    },
    reconnectDelay: 3000,
  ...
  const totals = useMemo(() => ({
    total: assets.length,
    ready: assets.filter((asset) => ['safe', 'hybrid'].includes(asset.pqc_status ?? '')).length,
    ...
    <div className="flex-1 overflow-y-auto bg-background-light p-8 dark:bg-background-dark">
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4 text-slate-900 dark:text-slate-100">
        <div className="flex items-center gap-4">
          <h1 className="text-3xl font-black tracking-tight">Master Asset Matrix</h1>
        ...
        <div className="flex items-center gap-2 rounded-lg bg-slate-100 p-1 dark:bg-primary/10">
          <button onClick={() => setViewMode('table')} className={`rounded px-4 py-2 text-sm font-bold transition-colors ${viewMode === 'table' ? 'bg-white text-primary shadow-sm dark:bg-primary/20' : 'text-slate-500'}`}>Table</button>
          <button onClick={() => setViewMode('topological')} className={`rounded px-4 py-2 text-sm font-bold transition-colors ${viewMode === 'topological' ? 'bg-white text-primary shadow-sm dark:bg-primary/20' : 'text-slate-500'}`}>Topology</button>
        </div>
      </div>

      {error && <div className="mb-6 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600 dark:border-red-500/20 dark:bg-red-500/10 dark:text-red-300">{error}</div>}
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 text-xs uppercase text-slate-500 dark:bg-slate-800/50 dark:text-slate-400">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold">TLS Version</th>
                <th className="px-6 py-4 font-semibold">Cipher</th>
                <th className="px-6 py-4 font-semibold">PQC Status</th>
                <th className="px-6 py-4 font-semibold">Risk Score</th>
                <th className="px-6 py-4 font-semibold">Last Scanned</th>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {assets.map((asset) => (
                <tr key={asset.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                  <td className="px-6 py-4 font-mono text-xs">{asset.asset}</td>
                  <td className="px-6 py-4">{asset.tls_version ?? 'Unknown'}</td>
                  <td className="px-6 py-4 font-mono text-xs">{asset.cipher ?? 'Unknown'}</td>
                  <td className="px-6 py-4 capitalize">{(asset.pqc_status ?? 'unknown').replace(/_/g, ' ')}</td>
                  ...
                  <td className="px-6 py-4 text-slate-500">{asset.last_scanned_at ? new Date(asset.last_scanned_at).toLocaleString() : 'Not scanned yet'}</td>
                  <td className="px-6 py-4">
                    <span className="rounded-full bg-slate-100 px-3 py-1 text-[10px] font-bold uppercase dark:bg-slate-800">{asset.status}</span>
                  </td>
                </tr>
              ))}
            </tbody>
